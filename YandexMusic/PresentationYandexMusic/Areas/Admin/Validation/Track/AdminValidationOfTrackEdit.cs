...
{
    public class AdminValidationOfTrackEdit : AbstractValidator<EditTrackViewModel>
    {
        ...
        public AdminValidationOfTrackEdit(ITrackDomainService trackDomain,
            ...
            IGenreDomainService genreDomain)
        {
            ...
            RuleFor(x => x.TrackName)
                .NotEmpty().WithMessage("Введитете имя трека")
            ...
            RuleFor(x => x)
                .Must(EditTrackIsUnique).WithMessage("Трек с таким именем уже существует");
        }

        public bool EditTrackIsUnique(EditTrackViewModel viewModel)
        {
            return trackDomain.EditTrackIsUnique(viewModel.Id, viewModel.TrackName);
        }
